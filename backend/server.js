...
const { sendContactReply } = require("./mailer");
...
app.post("/api/admin/contacts/:id/reply", adminOnly, async (req, res) => {
  ...
  try {
    ...
    await sendContactReply({
      toName: contact.first,
      toEmail: contact.email,
      originalMessage: contact.message,
      replyText: replyText.trim(),
    });

    await Contact.findByIdAndUpdate(req.params.id, { repliedAt: new Date() });
    res.json({ ok: true });
  } catch (err) {
